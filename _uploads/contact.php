<?php
/*
Template Name: MySelf - CV/Resume HTML Template
...
/* ReCaptch Secret */
$recaptchaSecret = '6Le9_N4UAAAAALwRd6FLmKv0oWdqArDUrhYs4FzN';

$dzEmailTo 		= "salvatore.giannini79@gmail.com";   /* Receiver Email Address */
$dzEmailFrom    = "Nwk Personal Site Contact";
...
try {
    if (!empty($_POST)) {
		...
		if($_POST['dzToDo'] == 'Contact')
		{
			...
			$dzMailSubject = 'Anthurium|nuovo contatto';
			$dzMailMessage	= 	"
								Questa persona vuole mettersi in contatto con te: <br><br>
								Name: $dzName<br/>
								Email: $dzEmail<br/>
			...
			{
				$dzRes['status'] = 1;
				$dzRes['msg'] = 'Abbiamo ricevuto il tuo messaggio con successo. Grazie per il contatto.';
			}
			else
			{
				$dzRes['status'] = 0;
				$dzRes['msg'] = 'Qualche problema nell'invio della posta, riprova più tardi.';
			}
			echo json_encode($dzRes);
			exit;
		}
...
<div class="g-recaptcha" data-sitekey="6Le9_N4UAAAAAA0j8N19nmeZM3_fgAEa3Iq43DMk" data-callback="verifyRecaptchaCallback" data-expired-callback="expiredRecaptchaCallback"></div>


			$dzMailSubject = 'MySelf|Appointment Form: A Person want to contact';
			$dzMailMessage	= 	"
